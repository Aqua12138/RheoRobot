...
class GridSensor3D(GridSensor):
    def __init__(self, SensorName, AgentGameObject, ObservationStacks, CellArc, LatAngleNorth, LatAngleSouth, LonAngle, MaxDistance, MinDistance, DistanceNormalization, n_particles):
    ...
    def normal_distance_particle(self, particle_state: ti.types.ndarray()):
        ...
        for p in range(self.n_particles):
            if self.particle_state_RL[p].distance < self.m_MaxDistance and self.particle_state_RL[p].distance > self.m_MinDistance:

            # 2. 判断经度范围和纬度范围
                if (90 - self.particle_state_RL[p].latitudes < self.m_LatAngleNorth and 90 - self.particle_state_RL[p].latitudes >= 0) or \
                (ti.abs(self.particle_state_RL[p].latitudes - 90) < self.m_LatAngleSouth and ti.abs(self.particle_state_RL[p].latitudes - 90) >= 0):
                    if ti.abs(self.particle_state_RL[p].longitudes) < self.m_LonAngle:
                        ...
                        if self.m_DistanceNormalization == 1:
                            ...
                            normal_d = 1 - d / (self.m_DistanceNormalization + ti.abs(d)) * (self.m_DistanceNormalization + 1)
                        # 计算经纬度索引
                        ...
                        ti.atomic_max(particle_state[longitude_index, latitude_index, self.particle_state_RL[p].tag], normal_d)
    @ti.kernel
                ...
                if (90 - self.mesh_state[I].latitudes < self.m_LatAngleNorth and 90 - self.mesh_state[
                    ...
                    if ti.abs(self.mesh_state[I].longitudes) < self.m_LonAngle:
                        ...
                        for i in ti.static(range(self.n_statics)):
                            ti.atomic_max(mesh_state[longitude_index, latitude_index, i], normal_d * self.statics[i].is_collide(self.mesh_state[I].x))

                        for i in ti.static(range(self.n_dynamics)):
                            ti.atomic_max(mesh_state[longitude_index, latitude_index, i+self.n_statics], normal_d * self.dynamics[i].is_collide(self.f, self.mesh_state[I].x))
    ...
    def get_obs(self, obs_type = "NormalDistance"):
        particle_state = np.zeros(((self.m_LonAngle // self.m_CellArc) * 2, (self.m_LatAngleNorth + self.m_LatAngleSouth) // self.m_CellArc, self.n_bodies), dtype=DTYPE_NP)
        mesh_state = np.zeros(((self.m_LonAngle // self.m_CellArc) * 2, (self.m_LatAngleNorth + self.m_LatAngleSouth) // self.m_CellArc, self.n_statics+self.n_dynamics), dtype=DTYPE_NP)
