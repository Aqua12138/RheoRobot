...
def _flatten_tensor_obs(obs: Union[List[VecEnvObs], Tuple[VecEnvObs]], space: gym.spaces.Space) -> VecEnvTensorObs:
    ...
    if isinstance(space, gym.spaces.Dict):
        ...
        return OrderedDict([(k, torch.stack([o[k] for o in obs])) for k in space.spaces.keys()])
    elif isinstance(space, gym.spaces.Tuple):
        assert isinstance(obs[0], tuple), "non-tuple observation for environment with Tuple observation space"
        obs_len = len(space.spaces)
        return tuple(torch.stack([o[i] for o in obs]) for i in range(obs_len))
    else:
...
class SHAC:
    def __init__(self, cfg):
        ...
        self.env = make_vec_env(env_id=cfg['params']['diff_env']['name'], n_envs=cfg["params"]["config"]["num_actors"], env_kwargs=env_kwargs, vec_env_cls=CustomSubprocVecEnv)

        print('num_envs = ', self.env.num_envs)
        print('num_actions = ', self.env.action_space)
        print('num_obs = ', self.env.observation_space)
        ...
        self.num_batch = cfg['params']['config'].get('num_batch', 4) # 一波steps_num分成几次进去
        self.batch_size = self.num_envs * self.steps_num // self.num_batch
        self.name = cfg['params']['config'].get('name', "Ant")

        self.truncate_grad = cfg["params"]["config"]["truncate_grads"] # 是否clip grad
        ...
        for i in range(self.steps_num):
            ...
            with torch.no_grad():
                ...
                if self.episode_length==self.max_episode_length[0]:
                    self.episode_loss_meter.update(self.episode_loss)
                    self.episode_discounted_loss_meter.update(self.episode_discounted_loss)

                    self.episode_length_meter.update(episode_length)
                    for done_env_id in range(self.num_envs):
                        ...
                        self.episode_length_his.append(episode_length[done_env_id].item())
